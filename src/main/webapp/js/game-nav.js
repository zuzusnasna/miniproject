...
/**
 * 장르별 게임 메뉴를 HTML로 생성합니다.
 */
function renderGameNav(categoryList, menuData) {
    categoryList.innerHTML = menuData
        .map(group => `
                    ...
                    ${group.games.map(game => `
                        ...
                        >
                            <span class="game-nav-game">
                                <img
                                    class="game-nav-icon"
                                    src="${escapeGameNav(resolveGameIconPath(game.iconUrl))}"
...
function injectGameNavStyles() {
    ...
    style.textContent = `
        ...
        /* 첫 번째 메뉴는 왼쪽을 기준으로 펼칩니다. */
        .game-nav-item:first-child .game-nav-dropdown {
            left: 0 !important;
            transform: translate(0, 0) !important;
        }

        /* 마지막 메뉴는 오른쪽을 기준으로 펼칩니다. */
        .game-nav-item:last-child .game-nav-dropdown {
            left: auto !important;
            right: 0 !important;
            transform: translate(0, 0) !important;
        }
...
/**
 * DB에 저장된 기존 Context Path를
 * 현재 애플리케이션의 Context Path로 변환합니다.
 */
...
/**
 * 게임 네비게이션에 표시되는 문자열의 HTML 특수문자를 이스케이프합니다.
 */
