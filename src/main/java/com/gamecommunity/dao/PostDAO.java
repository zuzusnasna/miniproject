...
public class PostDAO {
    ...
    public List<PostDTO> findAll() {
        ...
        String sql = "SELECT P.POST_ID, P.CATEGORY_ID, P.MEMBER_NO, M.USERNAME, M.NICKNAME, " +
                "P.TITLE, P.CONTENT, P.VIEW_COUNT, P.LIKE_COUNT, P.DISLIKE_COUNT, " +
                "P.IS_NOTICE, P.IS_DELETED, TO_CHAR(P.CREATED_AT, 'YYYY-MM-DD HH24:MI:SS') AS CREATED_AT " +
                "FROM POST P JOIN MEMBER M ON P.MEMBER_NO = M.MEMBER_NO " +
                "WHERE P.IS_DELETED = 'N' ORDER BY P.POST_ID DESC";
    ...
    public List<PostDTO> findByCategoryId(long categoryId) {
        ...
        String sql = "SELECT P.POST_ID, P.CATEGORY_ID, P.MEMBER_NO, M.USERNAME, M.NICKNAME, " +
                ...
                "P.IS_NOTICE, P.IS_DELETED, TO_CHAR(P.CREATED_AT, 'YYYY-MM-DD HH24:MI:SS') AS CREATED_AT " +
                "FROM POST P JOIN MEMBER M ON P.MEMBER_NO = M.MEMBER_NO " +
                "WHERE P.IS_DELETED = 'N' AND P.CATEGORY_ID = ? ORDER BY P.POST_ID DESC";
    ...
    public boolean save(PostDTO post) {
        String sql = "INSERT INTO POST (POST_ID, CATEGORY_ID, MEMBER_NO, TITLE, CONTENT, " +
    ...
    public boolean update(PostDTO post) {
        String sql = "UPDATE POST SET TITLE = ?, CONTENT = ?, UPDATED_AT = SYSDATE " +
    ...
    public List<PostDTO> findNotices() {
        ...
        String sql = "SELECT P.POST_ID, P.CATEGORY_ID, P.MEMBER_NO, M.USERNAME, M.NICKNAME, " +
                ...
                "P.IS_NOTICE, P.IS_DELETED, TO_CHAR(P.CREATED_AT, 'YYYY-MM-DD HH24:MI:SS') AS CREATED_AT " +
                "FROM POST P JOIN MEMBER M ON P.MEMBER_NO = M.MEMBER_NO " +
                "WHERE P.IS_DELETED = 'N' AND P.IS_NOTICE = 'Y' ORDER BY P.POST_ID DESC";
        try (Connection conn = DBUtil.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql); ResultSet rs = pstmt.executeQuery()) {
